...
export class Atom {
   ...
   */
  addNeighbouringElement(element: string) {
   ...
   */
  addAnchoredRing(ringId: number) {
    if (
      !ArrayHelper.contains(this.anchoredRings, {
        value: ringId,
      })
    ) {
   ...
   */
  haveCommonRingbond(atomA: Atom, atomB: Atom) {
   ...
   */
  neighbouringElementsEqual(arr: string[]) {
