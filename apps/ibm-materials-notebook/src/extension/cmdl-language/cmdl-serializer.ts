...
export class CMDLSerializer {
  ...
  public createGeneratedPolymerCell(
    ...
    try {
      cmdlLogger.silly(`serializing row ${rowNumber}...`);
      const cellText = this.serializeGeneratedPolymer(rowItem, rowNumber);
      cmdlLogger.info(`completed serialization for row ${rowNumber}`);
  ...
  private createContainer(containerGroup: string[], rowNumber: number) {
    try {
      ...
      const connections = containerGroup
        ...
        .join("\t\t");
      const footer = `\t};`;
      cmdlLogger.debug(`created a container group for ${containerNode}`);
  ...
  private sortGroups(nodeGroups: string[][]) {
    try {
      ...
      cmdlLogger.debug(`completed group sorting`, { meta: groups });
      return groups;
    } catch (error) {
      throw new Error(
  ...
  private parseGraphStr(graphStr: string) {
    try {
      ...
      cmdlLogger.silly(`split groups`, { meta: splitGroups });
      const nodeGroups = splitGroups.map((group) => {
        return group.split(";");
      });

      cmdlLogger.debug(`completed parsing of graph string...`, {
        meta: nodeGroups,
      });
      return nodeGroups;
    } catch (error) {
