...
export class Vertex {
   ...
   */
  constructor(value: Atom, x = 0, y = 0) {
    this.id = null;
    this.value = value;
   ...
   *
   */
  setPositionFromVector(v: Vector2) {
    this.position.x = v.x;
    this.position.y = v.y;
  }
   ...
   */
  addRingbondChild(vertexId: number, ringbondIndex: number) {
    this.children.push(vertexId);
   ...
   */
  setParentVertexId(parentVertexId: number) {
    this.neighbourCount++;
   ...
   */
  getAngle(referenceVector: Vector2 | null = null, returnAsDegrees = false) {
    let u = null;
   ...
   */
  getNeighbours(vertexId: number | null = null) {
    if (vertexId === null) {
